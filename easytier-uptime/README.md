...
`easytier-uptime` 作为独立的监控服务运行，提供：
- Web 管理界面
- REST API
- 本地数据库存储
- 节点健康检查和统计
...
使用 `neo-uptime-node` 作为独立的分布式探测节点：
- 完全独立的二进制程序，位于单独的 crate 中
- 只通过 HTTP API 与后端通信
- 不依赖 easytier-uptime 的代码
- 自动计算和上报延迟统计（RTT）
- 支持环境变量配置，易于容器化部署
- 详细使用说明请参考 [neo-uptime-node 目录](../neo-uptime-node/README.md)
